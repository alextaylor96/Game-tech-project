...
{	
	...
	float constraintMass = (pnodeA->GetInverseMass() + pnodeB->GetInverseMass()) +
		Vector3::Dot(c.colNormal, Vector3::Cross(pnodeA->GetInverseInertia() * Vector3::Cross(r1, c.colNormal), r1) +
	...
	if(tangent_len > 1e-6f){
		...
		float frictionalMass = (pnodeA->GetInverseMass() + pnodeB->GetInverseMass()) + Vector3::Dot(tangent, Vector3::Cross(pnodeA->GetInverseInertia()
			* Vector3::Cross(r1, tangent), r1) + Vector3::Cross(pnodeB->GetInverseInertia() * Vector3::Cross(r2, tangent), r2));
...
void Manifold::UpdateConstraint(ContactPoint& c)
{
	...
	const float elatisity_term = Vector3::Dot(c.colNormal, pnodeA->GetLinearVelocity() + Vector3::Cross(c.relPosA, pnodeA->GetAngularVelocity())
